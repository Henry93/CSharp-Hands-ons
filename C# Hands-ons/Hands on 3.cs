...
namespace LoopsMethodsExceptionsArraysClasses
{
    class Program
    {
        ...
        static int MultiplytTwoNumbers(int x, int y)
        {
            return  x * y;
        }
        ...
        void LoopOverParams(params int[] array) 
        {
        ...
        static void Main(string[] args)
        {
            ...
            var product = MultiplytTwoNumbers(3, 4);
            WriteLine(product);
            ...
            int[,] multiplicationTable = new int[4, 4]; // four numbers times four numbers
            
            for (int i = 0; i <= multiplicationTable.GetUpperBound(0); i++)
            {
                for (int j = 0; j <= multiplicationTable.GetUpperBound(1); j++)
                {

                    int prd = i * j;

                    if (j == multiplicationTable.GetUpperBound(1)) // if j equals the end of an array element
                    {
                       Write(prd + "\r\n"); // go to the new line
                    }
                    else
